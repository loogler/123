...
import com.qingyuan.util.pulldown.ScrollOverListView.OnScrollOverListener;
...
public class PullDownView extends LinearLayout implements OnScrollOverListener{
	...
	private void initHeaderViewAndFooterViewAndListView(Context context){
		...
		mHeaderView = LayoutInflater.from(context).inflate(R.layout.item_listview_head, null);
		...
		//自定义脚部文件
		mFooterView = LayoutInflater.from(context).inflate(R.layout.item_listview_foot, null);
	...
	*/
	private void checkHeaderViewState(){
		...
		mHeaderArrowView.startAnimation(mRotateOTo180Animation);
			...
			if(mHeaderViewState == HEADER_VIEW_STATE_NOT_OVER_HEIGHT || mHeaderViewState == HEADER_VIEW_STATE_IDLE) return;
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
				case WHAT_DID_LOAD_DATA:{
						...
						mHeaderTextView.setText("下拉可以刷新");
						mHeaderViewTextView = (TextView) mHeaderView.findViewById(R.id.pulldown_header_date);
						mHeaderViewTextView.setVisibility(View.VISIBLE);
						mHeaderViewTextView.setText("更新于：" + dateFormat.format(new Date(System.currentTimeMillis())));
