...
public class L_VisitAdapter extends BaseAdapter {
	private LayoutInflater inflater;
	public static List<TableItem_Visit> mData;
	Context context;
	AsyncImageLoader2 asyncImageLoader2 = new AsyncImageLoader2();

	public L_VisitAdapter(Context context, List<TableItem_Visit> mList) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView != null) {
			...
			imgUrl = "http://www.07919.com/"
					+ mData.get(position).getUser_pic();
			tv_recent_unread.setVisibility(View.GONE);
			tv_recent_time.setText(mData.get(position).getTime());
			tv_recent_name.setText("会员 " + mData.get(position).getUser_nick());
			loadImage(imgUrl, R.id.iv_recent_avatar, view);
			tv_recent_msg.setText(mData.get(position).getUser_info());
		}
